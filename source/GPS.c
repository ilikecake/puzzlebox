...
portTASK_FUNCTION(vGPSTask, pvParameters) {
	...
	while (1)
	{
		...
		if (GPSBuffer[datacount] == ',')//Identify type of data string     //All GPS return strings are terminated by <CR><LF>
		{
...
//				printf(")");
				...
				{//$GPRMC,111636.932,A,2447.0949,N,12100.5223,E,000.0,000.0,030407,,,A*61<CR><LF>
					...
					{ //UTC Time
						...
						if (datacount>=9) //7, 8, 9 are digits after decimal place in seconds
...
//						}
				...
				{//$GPGLL,2447.0944,N,12100.5213,E,112609.932,A,A*57<CR><LF>
					...
					{  //Latitude
						...
						if (datacount>=8) //5, 6, 7, 8 are digits after decimal place in minutes
						{
...
//						printf(", Altitude: %f\r\n", GPSData.ALT);
		...
		else //GPS data is in the middle of a term
		{
			...
			if (datacount>70)
			{
				datacount=0;//prevent datacount from exceeding maximum GPSBuffer index in the event of an input error
			}
		}
